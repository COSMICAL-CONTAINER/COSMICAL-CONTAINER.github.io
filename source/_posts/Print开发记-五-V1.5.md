---
title: Print.h 开发记（五）V1.5：覆盖基础类型
tags:
  - C语言
...
羡慕 Python 的 `print()` 吗？这个系列记录我用纯 C 实现一个万能 print 库的全过程——**项目已开源**：

> **GitHub**：<https://github.com/COSMICAL-CONTAINER/C_Program-Print.h>

V1.5 是一次"夯实基础"的版本，没有新花样，主要做了三件事：**补全基础类型与指针类型的打印**、**规范缓冲区与命名**、**完善注释**。库从这个版本开始有了"正式出品"的样子。

## 基础类型覆盖：分发表补全

`_Generic` 分发表补上了缺的三种基础类型——`unsigned char`、`unsigned int`、`long long`，各自对应一个新的打印函数：
...
加上 V1.2 就有的 `unsigned long long`，常用类型到此全部覆盖。

## 缓冲区与命名规范化

各类型转字符串的静态缓冲区，V1.5 按实际宽度逐个分配大小，旁边还注明了该类型的字节范围；总长度宏也从 `MAXSTRLEN` 改成了更清晰的 `PRINTMAXSTRLEN`：
...
以后想检查"这个缓冲区会不会溢出"，看注释就知道。

## 注释规范化

头文件加上了规范的 Doxygen 注释头——作者、邮箱、GitHub 地址、版本号、日期一应俱全，`@see` 区还维护着每个版本的更新日志。打开文件就知道这是谁的什么项目、每一版都干了什么。
...
- 上一篇：[Print.h 开发记（四）V1.4](/posts/4c8a91b3/)
- 下一篇：[V1.6 debug 宏家族](/posts/6e9f2a48/)
