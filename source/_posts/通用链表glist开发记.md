...
这个项目最后叫 glist（general list），从 V1.0 迭代到 V1.2。三个版本不多，恰好是 C 语言里实现"泛型"的三条思路的进化史：**只存地址 → 给类型发身份证 → 让数据自己带说明书**。
...
两条路没有高下：`_Generic` 快，但类型集合编译时就冻死了；函数指针慢一拍，换来的是库的门永远开着。C 语言没有泛型语法，但它把实现泛型的零件全都发到了你手上——glist 这三个版本，就是把这些零件一个个捡起来的过程。
